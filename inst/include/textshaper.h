...
static inline int string_shape(const char* string, const char* fontfile, int index,
                               ...
                               int* id, unsigned int max_length) {
  static int (*p_string_shape)(const char*, const char*, int, double, double, double*, double*, int*, unsigned int) = NULL;
  if (p_string_shape == NULL) {
    p_string_shape = (int (*)(const char*, const char*, int, double, double, double*, double*, int*, unsigned int)) R_GetCCallable("systemfonts", "string_shape");
  }
  return p_string_shape(string, fontfile, index, size, res, x, y, id, max_length);
}
